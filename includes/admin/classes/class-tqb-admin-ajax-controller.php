		...
		if ( ( $custom = $this->param( 'custom' ) ) ) {
			switch ( $custom ) {
				...
				case 'reset_statistics':
					...
					if ( ! $stats_reset ) {
						return $this->error( __( 'Could not reset quiz statistics', Thrive_Quiz_Builder::T ) );
					}
		...
		switch ( $method ) {
			...
			case 'PATCH':
				...
				if ( ! empty( $model['type'] ) && ! empty( $model['ID'] ) ) {
					...
					} else if ( $model['type'] === Thrive_Quiz_Builder::QUIZ_TYPE_RIGHT_WRONG ) {
						TQB_Post_meta::update_quiz_highlight_answer_meta( $model['ID'], ! empty( $model['highlight_answer'] ) ? $model['highlight_answer'] : false );
					}
